...
def evaluate_dinov3_model(test_indices, label_mapping, device):
    """DINOv3 RoMa 모델을 평가합니다."""
    
    print(f"\n{'='*80}")
    print(f"🔄 DINOv3 RoMa 평가 시작")
    print('='*80)
    ...
    try:
        ...
        model.eval()
        print(f"✅ DINOv3 RoMa 모델 로드 완료")
        
        all_results = []
        
        for pair_idx in tqdm(test_indices, desc="DINOv3 RoMa 평가"):
            try:
                # 이미지 로드
                ...
                if metrics:
                    metrics['model_name'] = 'DINOv3 RoMa'
        ...
        print(f"✅ DINOv3 RoMa 평가 완료: {len(all_results)}개 샘플")
        return all_results
        
    finally:
        os.chdir(original_cwd)

def evaluate_original_model(test_indices, label_mapping, device):
    """원본 RoMa 모델을 평가합니다."""
    
    print(f"\n{'='*80}")
    print(f"🔄 원본 RoMa (DINOv2) 평가 시작")
    print('='*80)
    ...
    try:
        ...
        print(f"✅ 원본 RoMa (DINOv2) 모델 로드 완료")
        
        all_results = []
        
        for pair_idx in tqdm(test_indices, desc="원본 RoMa 평가"):
            try:
                # 이미지 로드
                pair_dir = os.path.join('../processed_refine_data', f'pair_{pair_idx:04d}')
                img_source = cv2.cvtColor(cv2.imread(os.path.join(pair_dir, 'google.jpg')), cv2.COLOR_BGR2RGB)
                ...
                metrics = calculate_metrics(H_pred, gt_source_pts, gt_target_pts)
                if metrics:
                    metrics['model_name'] = '원본 RoMa (DINOv2)'
                    all_results.append(metrics)
        ...
        print(f"✅ 원본 RoMa (DINOv2) 평가 완료: {len(all_results)}개 샘플")
        return all_results
...
def compare_models():
    ...
    original_results = evaluate_original_model(test_indices, label_mapping, device)
    dinov3_results = evaluate_dinov3_model(test_indices, label_mapping, device)
